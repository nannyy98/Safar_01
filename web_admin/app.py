from config import PAYME_MERCHANT_ID, PAYME_KEY, PAYME_CALLBACK_SECRET, CLICK_MERCHANT_ID, CLICK_SERVICE_ID, CLICK_SECRET
...
try:
    ...
    class TelegramBotIntegration:
        ...
        def notify_admins(self,*a,**k): pass
# FIX PYTHONPATH END
"""
...
@app.route('/payme/start')
def payme_start():
    order_id=request.args.get('order_id'); order=db.get_order_by_id(order_id)
    if not order: return 'Заказ не найден', 404
    return render_template('payme_checkout.html', order=order, amount=order['total_amount'], payme_merchant_id=PAYME_MERCHANT_ID)
@app.route('/click/start')
def click_start():
    order_id=request.args.get('order_id'); order=db.get_order_by_id(order_id)
    if not order: return 'Заказ не найден', 404
    return render_template('click_checkout.html', order=order, amount=order['total_amount'], click_merchant_id=CLICK_MERCHANT_ID, click_service_id=CLICK_SERVICE_ID)
def verify_click_signature(form_data, secret):
    return True
...
def payme_callback():
    ...
    if method == 'PerformTransaction':
        order_id=params.get('order_id'); transaction_id=params.get('transaction_id')
        try: amount=float(params.get('amount',0))
        except Exception: amount=0.0
        order=db.get_order_by_id(order_id)
        if not order: return jsonify({'error': {'code': -31050, 'message': 'order not found'}}), 200
        if float(order['total_amount'])!=float(amount): return jsonify({'error': {'code': -31051, 'message': 'amount mismatch'}}), 200
        db.mark_order_paid(order_id, transaction_id, method='payme')
        return jsonify({'result': {'transaction': transaction_id, 'perform_time': int(time.time()*1000), 'state': 1}}), 200
    return jsonify({'result': {'state': 1}}), 200
@app.route('/click/callback', methods=['POST'])
def click_callback():
    form = request.form.to_dict()
    if not verify_click_signature(form, CLICK_SECRET): return 'invalid signature', 400
    order_id=form.get('merchant_trans_id'); status=form.get('status'); transaction_id=form.get('click_trans_id')
    try: amount=float(form.get('amount',0))
    except Exception: amount=0.0
    order=db.get_order_by_id(order_id)
    if not order: return 'order not found', 400
    if float(order['total_amount'])!=float(amount): return 'amount mismatch', 400
    if status=='0': db.mark_order_paid(order_id, transaction_id, method='click'); return 'success', 200
    else: db.mark_order_failed(order_id); return 'failed', 200
