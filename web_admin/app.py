...
CANDIDATES = [REPO_ROOT, os.path.join(REPO_ROOT, "project")]
for p in CANDIDATES:
...
def payme_start():
    ...
    return render_template('payme_checkout.html', order=order, amount=amount, payme_merchant_id=PAYME_MERCHANT_ID)
...
def click_start():
    ...
    if not order:
        return "Заказ не найден", 404
    amount = order["total_amount"]
    return render_template('click_checkout.html', order=order, amount=amount, click_merchant_id=CLICK_MERCHANT_ID, click_service_id=CLICK_SERVICE_ID)
...
def payme_callback():
    ...
    if method == 'PerformTransaction':
        ...
        order = db.get_order_by_id(order_id)
        if not order:
            return jsonify({"error": {"code": -31050, "message": "order not found"}}), 200
        if float(order["total_amount"]) != float(amount):
            return jsonify({"error": {"code": -31051, "message": "amount mismatch"}}), 200
...
def click_callback():
    ...
    if not verify_click_signature(form, CLICK_SECRET):
        return "invalid signature", 400
    order_id = form.get('merchant_trans_id')
    amount = form.get('amount')
    status = form.get('status')
    transaction_id = form.get('click_trans_id')
    order = db.get_order_by_id(order_id)
    if not order:
    ...
    if status == '0':
        db.mark_order_paid(order_id, transaction_id, method='click')
        return "success", 200
    else:
        db.mark_order_failed(order_id, reason=f"click status {status}")
        return "failed", 200
